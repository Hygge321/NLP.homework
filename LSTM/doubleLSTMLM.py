...
class TextLSTM(nn.Module):
    def __init__(self, input_size: int, hidden_size: int):
        ...
        self.embeding = nn.Embedding(n_class, embedding_dim=input_size)
        # f_t
        # forget gates
        self.W_f1 = nn.Parameter(torch.Tensor(input_size, n_hidden))
        self.W_f2 = nn.Parameter(torch.Tensor(n_hidden, n_hidden))
        self.b_f1 = nn.Parameter(torch.Tensor(n_hidden))
        self.b_f2 = nn.Parameter(torch.Tensor(n_hidden))

        # i_t
        # input gates
        self.W_i1 = nn.Parameter(torch.Tensor(input_size, n_hidden))
        self.W_i2 = nn.Parameter(torch.Tensor(n_hidden, n_hidden))
        self.b_i1 = nn.Parameter(torch.Tensor(n_hidden))
        self.b_i2 = nn.Parameter(torch.Tensor(n_hidden))

        # g_t
        # cell gates
        self.W_g1 = nn.Parameter(torch.Tensor(input_size, n_hidden))
        self.W_g2 = nn.Parameter(torch.Tensor(n_hidden, n_hidden))
        self.b_g1 = nn.Parameter(torch.Tensor(n_hidden))
        self.b_g2 = nn.Parameter(torch.Tensor(n_hidden))

        # o_t
        # output gates
        self.W_o1 = nn.Parameter(torch.Tensor(input_size, n_hidden))
        self.W_o2 = nn.Parameter(torch.Tensor(n_hidden, n_hidden))
        self.b_o1 = nn.Parameter(torch.Tensor(n_hidden))
        self.b_o2 = nn.Parameter(torch.Tensor(n_hidden))
        #################################################################################
        # f_t
        # forget gates
        self.W_f1_2 = nn.Parameter(torch.Tensor(n_hidden, n_hidden))
        self.W_f2_2 = nn.Parameter(torch.Tensor(n_hidden, n_hidden))
        self.b_f1_2 = nn.Parameter(torch.Tensor(n_hidden))
        self.b_f2_2 = nn.Parameter(torch.Tensor(n_hidden))

        # i_t
        # input gates
        self.W_i1_2 = nn.Parameter(torch.Tensor(n_hidden, n_hidden))
        self.W_i2_2 = nn.Parameter(torch.Tensor(n_hidden, n_hidden))
        self.b_i1_2 = nn.Parameter(torch.Tensor(n_hidden))
        self.b_i2_2 = nn.Parameter(torch.Tensor(n_hidden))

        # g_t
        # cell gates
        self.W_g1_2 = nn.Parameter(torch.Tensor(n_hidden, n_hidden))
        self.W_g2_2 = nn.Parameter(torch.Tensor(n_hidden, n_hidden))
        self.b_g1_2 = nn.Parameter(torch.Tensor(n_hidden))
        self.b_g2_2 = nn.Parameter(torch.Tensor(n_hidden))

        # o_t
        # output gates
        self.W_o1_2 = nn.Parameter(torch.Tensor(n_hidden, n_hidden))
        self.W_o2_2 = nn.Parameter(torch.Tensor(n_hidden, n_hidden))
        self.b_o1_2 = nn.Parameter(torch.Tensor(n_hidden))
        self.b_o2_2 = nn.Parameter(torch.Tensor(n_hidden))
    ...
    def forward(self, X):
        outputs = []
        # 输入 : [batch size,n_step]
        X = self.embeding(X)
        batch_size, n_step, embed = X.size()

        # hidden_state 为短期记忆状态 [batch_size, n_hidden]
        hidden_state = torch.ones(batch_size, self.hidden_size).to(X.device)
        # cell_state 为长期记忆状态 [batch_size, n_hidden]
        cell_state = torch.ones(batch_size, self.hidden_size).to(X.device)

        hidden_state_2 = torch.ones(batch_size, self.hidden_size).to(X.device)
        cell_state_2 = torch.ones(batch_size, self.hidden_size).to(X.device)

        # X : [batch_size,n_step , embeding size]
        for t in range(n_step):
            x_t = X[:, t, :]
            # forget gate
            f_t = torch.sigmoid(x_t @ self.W_f1 + self.b_f1 + hidden_state @ self.W_f2 + self.b_f2)
            # input gate
            i_t = torch.sigmoid(x_t @ self.W_i1 + self.b_i1 + hidden_state @ self.W_i2 + self.b_i2)
            g_t = torch.tanh(x_t @ self.W_g1 + self.b_g1 + hidden_state @ self.W_g2 + self.b_g2)
            # Memory update
            cell_state = f_t * cell_state + i_t * g_t
            # output gate
            o_t = torch.sigmoid(x_t @ self.W_o1 + self.b_o1 + hidden_state @ self.W_o2 + self.b_o2)
            hidden_state = o_t * torch.tanh(cell_state)
            #################################################################################
            # forget gate
            f_t_2 = torch.sigmoid(hidden_state @ self.W_f1_2 + self.b_f1_2 + hidden_state_2 @ self.W_f2_2 + self.b_f2_2)
            # input gate
            i_t_2 = torch.sigmoid(hidden_state @ self.W_i1_2 + self.b_i1_2 + hidden_state_2 @ self.W_i2_2 + self.b_i2_2)
            g_t_2= torch.tanh(hidden_state @ self.W_g1_2 + self.b_g1_2 + hidden_state_2 @ self.W_g2_2 + self.b_g2_2)
            # Memory update
            cell_state_2 = f_t_2 * cell_state_2 + i_t_2 * g_t_2
            # output gate
            o_t_2 = torch.sigmoid(hidden_state @ self.W_o1_2 + self.b_o1_2 + hidden_state_2 @ self.W_o2_2 + self.b_o2_2)
            hidden_state_2 = o_t_2 * torch.tanh(cell_state_2)



            # save the state after each cycle
            outputs.append(hidden_state_2.unsqueeze(0))
...
def train_LSTMlm():
    model = TextLSTM(input_size=emb_size, hidden_size=n_hidden)
    model.to(device)
    print(model)
